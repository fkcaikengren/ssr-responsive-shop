...
function Menu() {
  ...
  const categories = {
    0: ['汉服秦制', '汉服汉制', '汉服唐制', '汉服宋制', '汉服明制', '清朝服饰'],
    1: ['发髻', '簪子', '纱巾', '蝴蝶结'],
    2: ['翘头鞋', '登云履', '皂靴', '布帛袜'],
    3: ['标准旗袍', '改良旗袍'],
    4: [],
  ...
  return (
    <div className={`${style.menu}`}>
      <div className={`${style.menu}__header`}>
        <img
          className={`${style.menu}__logo`}
          src={logo}
          alt="ssr-responsive-shop"
        />
      ...
      <div className={`${style.menu}__body`}>
          ...
          <ul>
            {navs &&
              navs.map((item, i) => (
                <li
                  key={i}
                  className={activeIndex === i ? `${style.menu}__select` : ''}
                >
                  <span>{item}</span>
                </li>
              ))}
        ...
        <div className={`${style.menu}__main`}>nei rong</div>
      </div>
    </div>
  );
}
